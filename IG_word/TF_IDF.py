...
def tf_idf():
    ...
    filePathC = "cnew.txt"  # 从text文件中读取文件
    label, class_df_list, word_list=read_file(filePathC)
...
def test_tf_idf():
    vocabList = reduction_words()  # 降维后的词典
    filePathC = "new_test.txt"  # 每个文件名数组
    test_label, class_df_list, test_word_list = fenci_all(filePathC)
